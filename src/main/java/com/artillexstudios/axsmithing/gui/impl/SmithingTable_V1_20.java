...
public class SmithingTable_V1_20 implements SmithingTable, InventoryHolder {
    ...
    public void open(Player player) {
        ...
        Inventory inventory = Bukkit.createInventory(this,
                AxSmithingPlugin.getConfiguration().getInt("menu.1_20.rows") * 9,
                StringUtils.format(AxSmithingPlugin.getConfiguration().getString("menu.1_20.title")));
        Set<Object> items = AxSmithingPlugin.getConfiguration().getSection("menu.1_20.items").getKeys();

        for (Object item : items) {

            Section section = AxSmithingPlugin.getConfiguration().getSection("menu.1_20.items")
                    .getSection(item.toString());
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        if (!(event.getInventory().getHolder() instanceof SmithingTable_V1_20))
            return;

        if ((event.getSlot() == upgradeSlot || event.getSlot() == itemSlot || event.getSlot() == templateSlot)
                && event.getInventory().getItem(outputSlot) != null)
        {

            if (checkRecipes(event.getInventory(), event.getInventory().getItem(templateSlot),
                    event.getInventory().getItem(itemSlot), event.getInventory().getItem(upgradeSlot)))
            {
        ...
        if (event.getSlot() == outputSlot && event.getCurrentItem() != null && (event.getCursor() == null
        ...
        if (event.getSlot() == outputSlot && event.getView().getTopInventory().getItem(outputSlot) != null
                && event.getView().getTopInventory().getItem(outputSlot).getType() != Material.AIR)
        {
    ...
    public void handleClose(InventoryCloseEvent event) {
        ...
        if (event.getInventory().getItem(templateSlot) != null
                && event.getInventory().getItem(templateSlot).getType() != Material.AIR)
            event.getPlayer().getInventory().addItem(event.getInventory().getItem(templateSlot));
        if (event.getInventory().getItem(itemSlot) != null
                && event.getInventory().getItem(itemSlot).getType() != Material.AIR)
            event.getPlayer().getInventory().addItem(event.getInventory().getItem(itemSlot));
        if (event.getInventory().getItem(upgradeSlot) != null
                && event.getInventory().getItem(upgradeSlot).getType() != Material.AIR)
    ...
    public void handleDrag(InventoryDragEvent event) {
        ...
        if (event.getRawSlots().contains(outputSlot) && (outputItem == null || outputItem.getType().isAir())
    ...
    private boolean checkRecipe(Inventory inventory, ItemStack finalTemplate, ItemStack finalBase,
            ItemStack finalAddition)
    {
        ...
        while (recipeIterator.hasNext()) {
            ...
            if (recipe instanceof SmithingTrimRecipe trimRecipe) {
                ...
                if (finalBase == null || finalAddition == null || finalTemplate == null)
                    return false;
    ...
    private ArmorTrim armorTrim(@NotNull ItemStack finalAddition, @NotNull ItemStack finalTemplate) {
        ...
        pattern = Registry.TRIM_PATTERN.get(NamespacedKey.minecraft(finalTemplate.getType().name()
                .replace("_ARMOR_TRIM_SMITHING_TEMPLATE", "").toLowerCase(Locale.ENGLISH)));
