...
public class SmithingTable_V1_16 implements SmithingTable, InventoryHolder {
    ...
    public void open(Player player) {
        ...
        Inventory inventory = Bukkit.createInventory(this,
                AxSmithingPlugin.getConfiguration().getInt("menu.1_16.rows") * 9,
                StringUtils.format(AxSmithingPlugin.getConfiguration().getString("menu.1_16.title")));
        Set<Object> items = AxSmithingPlugin.getConfiguration().getSection("menu.1_16.items").getKeys();

        for (Object item : items) {

            Section section = AxSmithingPlugin.getConfiguration().getSection("menu.1_16.items")
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        if (!(event.getInventory().getHolder() instanceof SmithingTable_V1_16))
            return;

        if ((event.getSlot() == upgradeSlot || event.getSlot() == itemSlot)
                && event.getInventory().getItem(outputSlot) != null)
        {

            if (checkRecipes(event.getInventory(), event.getInventory().getItem(itemSlot),
                    event.getInventory().getItem(upgradeSlot)))
            {
    ...
    public void handleDrag(InventoryDragEvent event) {
        ...
        if (event.getInventory().getItem(outputSlot) != null
                && event.getInventory().getItem(outputSlot).getType() != Material.AIR)
        {
