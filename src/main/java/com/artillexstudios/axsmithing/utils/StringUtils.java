...
public class StringUtils {
    ...
    private static final LegacyComponentSerializer LEGACY_COMPONENT_SERIALIZER = LegacyComponentSerializer.builder()
            .character('\u00a7').useUnusualXRepeatedCharacterHexFormat().hexColors().build();
    private static final LegacyComponentSerializer LEGACY_FORMATTER = LegacyComponentSerializer.legacyAmpersand()
            .toBuilder().useUnusualXRepeatedCharacterHexFormat().hexColors().build();
    ...
    public static String format(@NotNull String message) {
        ...
        message = toLegacy(
                MINI_MESSAGE.deserialize(message).applyFallbackStyle(TextDecoration.ITALIC.withState(false)));
