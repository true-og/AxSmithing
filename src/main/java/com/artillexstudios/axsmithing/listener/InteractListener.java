...
public class InteractListener implements Listener {
    ...
    public void onPlayerInteractEvent(@NotNull final PlayerInteractEvent event) {
        ...
        if (event.getAction().equals(Action.LEFT_CLICK_BLOCK))
            return;
        if (!AxSmithingPlugin.getConfiguration().getBoolean("listen-to-interact-event"))
            return;

        if (AxSmithingPlugin.is1_20()) {

            if (Via.getAPI().getPlayerVersion(event.getPlayer()) >= 762
                    && !AxSmithingPlugin.getConfiguration().getBoolean("menu.1_20.force-for-1_20-clients"))
            {
